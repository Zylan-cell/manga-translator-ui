import { BoundingBox, TextItem } from "../../types";
  ...
  if (!maskMode) {
    ...
    if (editMode) {
      ...
      if (sel) {
        ...
        ctx.fillRect(x2 - s / 2, y1 - s / 2, s, s);
        ctx.fillRect(x1 - s / 2, y2 - s / 2, s, s);
        ctx.fillRect(x2 - s / 2, y2 - s / 2, s, s);
      }
    }
